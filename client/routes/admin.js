...
router.get('/courses', async (req, res) => {
  try {
    const courses = await Course.find().select('-lessons').sort({ createdAt: -1 });
    res.json(courses);
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.put('/courses/:id', async (req, res) => {
  try {
    const course = await Course.findByIdAndUpdate(req.params.id, req.body, { new: true }).select('-lessons');
...
// Upload cert file URL for a student
router.put('/certificates/:id/upload', async (req, res) => {
  try {
    const { fileUrl, adminSignature, issuedBy, issuedByTitle } = req.body;
    const cert = await Certificate.findByIdAndUpdate(
      req.params.id,
      { fileUrl, adminSignature, issuedBy, issuedByTitle, status: 'approved', approvedAt: new Date() },
      { new: true }
    ).populate('student', 'name email').populate('course', 'title');
    res.json(cert);
  } catch (err) { res.status(500).json({ message: err.message }); }
